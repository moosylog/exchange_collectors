...
function FTX_PrivateRequest(FTXrequest) {      
  ...
  var timestamp = new Date().getTime().toString(),                       
      payld     = timestamp + FTXrequest.method +val,    // not sure, needs testing    
   ...
   return  { uri: FTXrequest.uri + FTXrequest.command, params: params};
}


function FTX_PublicRequest(FTXrequest) {  
  if (FTXrequest.hasOwnProperty('payload')  === true) FTXrequest.payload  = CreateURIQueryString(FTXrequest.payload,"?");
        params    = {
       'method'            : FTXrequest.method,  
       'muteHttpExceptions': true,
       'headers': {
