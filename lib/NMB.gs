...
function NMB_PrivateRequest(stn) {
  ...
  if ( stn.payload != "") para = CreateURIQueryString(stn.payload,"&")
  
  //try {var pld = JSON.stringify(stn.payload);} catch(e) {var pld = "";}
  //if (pld.length < 4) pld = ""; 
  ...
  var aut = Utilities.base64Encode(stn.apikey+":"+stn.secret),
   ...
   return  { uri: stn.uri + stn.command +"?timestamp="+timestamp+"&receiveWindow=5000" + para, params: params};
}

function NMB_PublicRequest(stn) {
  var para = "";
  if (stn.hasOwnProperty('payload') === false) stn.payload = "";
  if ( stn.payload != "") para = CreateURIQueryString(stn.payload,"&")
